...
def intron_length(intron):
    return (intron[1]-intron[0])+1
...
for gene in geneRecords:                                            
    if len(gene.origGraph.isoforms()) > 1:                          
        gene_count += 1                                             
        gene_isoforms += len(gene.origGraph.isoforms())             
        intron_count += len(gene.introns)                           
        #introns.append(len(gene.introns))                          
        for intron in gene.introns:                                 
            introns.append(intron_length(intron))

print "Multi-transcript genes: ", gene_count
print "Transcripts per gene: ", gene_isoforms/float(gene_count)
print "Introns per gene: ", intron_count/float(gene_count)
print "Median intron length: ", np.median(introns)
print "Mean intron length: ",np.mean(introns)


#Command to Run
#for_stats_script.py -l Heat Control /s/jawar/a/nobackup/altsplice1/fahad/Human_hg19/Annotations/refGene.gtf /s/jawar/h/nobackup/fahad/Human_Chromatin/RNAseq/GM12878/wgEncode_R2x75Il200Aligns_Merged_noMulti.bam /s/jawar/h/nobackup/fahad/Human_Chromatin/RNAseq/GM12878/wgEncode_R2x75Il200Aligns_Merged_noMulti.bam -o extract_IR_test -v -k 2 8
