...
for gene in geneRecords:
	...
	Heat_Exp,Control_Exp,Heat_Junc,Control_Junc = 	getDepthsFromBamfiles(gene,nspace.factor1bamfiles,nspace.factor2bamfiles)
	Heat_Exp = Heat_Exp[0]
	Control_Exp = Control_Exp[0]
	Heat_Junc = Heat_Junc[0]
	Control_Junc = Control_Junc[0]
	if len(Heat_Exp)==0: #same for Control_Exp as both are for the same bamfile in this case
		continue #gene not found in bam
	#track_exp_heat.append(float(sum(Heat_Exp))/len(Heat_Exp))
	#track_exp_control.append(float(sum(Control_Exp))/len(Control_Exp))
	F1C_heat = numpy.array([(Heat_Exp[s:(e+1)]).mean() for s,e in gene.exons]).mean(0)
	F1C_control = numpy.array([(Control_Exp[s:(e+1)]).mean() for s,e in gene.exons]).mean(0)
	track_exp_heat[gene.gid] = F1C_heat
	track_exp_control[gene.gid] = F1C_control


final_list=[['Gene','Heat','Control']] #Heat and Control in this case will be the same thing (just using one sample)
for key in track_exp_control.keys():
    final_list.append([key,track_exp_heat[key],track_exp_control[key]])
#len(final_list)
np.savetxt('exonic_expression_list.txt',final_list,fmt='%s')
...
#numpy.savetxt('Union_IR_Events_Control-Heat.txt',events_list,fmt='%s',delimiter='\t')
