...
for i in range(0,len(exp_limit)):
	...
	##############Hypergeometric Test###################
	invP = hypergeom.cdf(IR[i]+1,IE_All_Pop[i],IR_Pop[i],IE_All[i])
	...
	Final_Res.append([ IR[i],IR_Pop[i],round(IR_Content[i]*100,2), IE[i],IE_Pop[i],round(IE_Content[i]*100,2), HypergeoPMF[i], invP])

np.savetxt(TF_name+'-some_res_StartInGene_noFirstIntron.txt',Final_Res,fmt='%s',delimiter='\t')
