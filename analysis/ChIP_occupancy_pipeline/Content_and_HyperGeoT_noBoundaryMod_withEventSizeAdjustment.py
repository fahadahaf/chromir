...
for i in range(0,len(exp_limit)):
	#########For IR#########
	fname = 'HotSpot_Data/'+TF_name+'_in_IR_'+exp_limit[i]+'_expressed_noBoundaryModified_StartInGene_intronCoverage.txt'
	...
	#########For IE##########
	fname = 'HotSpot_Data/'+TF_name+'_in_IE_Neg_'+exp_limit[i]+'_expressed_noBoundaryModified_StartInGene_intronCoverage.txt'
...
np.savetxt(TF_name+'-some_res_noBoundaryMod_EventLenAdj_StartInGene.txt',Final_Res,fmt='%s',delimiter='\t')
