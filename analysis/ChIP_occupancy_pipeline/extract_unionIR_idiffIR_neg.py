...
#numpy.savetxt('Union_IR_Events_Control-Heat_Neg.txt',events_list,fmt='%s',delimiter='\t')
######################################################################################


#############Use this code if extracting negative examples with genes without any IR event###############
#events_list = [['Gene_ID','Pre_Exon','Intron','Post_Exon','Strand']]
...
#numpy.savetxt('Union_IR_Events_Control-Heat_Neg_noIRgenes.txt',events_list,fmt='%s',delimiter='\t')
#########################################################################################################
...
for gene in geneRecords:
	...
	if len(ret)==len(gene.introns):
		for i in range(0,len(ret)):
			if ret[i]==False:
				#use below statement within the if (and remove if above to get all Neg events)
				events_list.append([gene.gid,str((gene.exons[i][0]+ref,gene.exons[i][1]+ref)),str((gene.introns[i][0]+ref,gene.introns[i][1]+ref-1)),str((gene.exons[i+1][0]+ref,gene.exons[i+1][1]+ref)),gene.strand])

numpy.savetxt('Union_IR_Events_Neg.txt',events_list,fmt='%s',delimiter='\t')
#########################################################################################################


#############Use this code if extracting ALL negative examples with genes with all IR and IE event###############
events_list_ALL = [['Gene_ID','Pre_Exon','Intron','Post_Exon','Strand']]
